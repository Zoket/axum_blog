...
pub async fn create(client: &Client, frm: &form::CreateTopic) -> Result<TopicID> {
    ...
    super::insert(
        ...
        &[&frm.title, &frm.category_id, &frm.summary, &frm.markdown, &html, &dateline],
        "添加文章失败",
    ).await
}
...
pub async fn archive_list(client: &Client) -> Result<Vec<TopicArchive>> {
    let sql = "SELECT to_char(DATE_TRUNC('month',dateline), 'YYYY年MM月') AS dateline FROM topics GROUP BY to_char(DATE_TRUNC('month',dateline), 'YYYY年MM月')";
...
pub async fn detail(client: &Client, id: i64) -> Result<TopicDetail> {
    super::execute(
        client,
        "UPDATE topics SET hit = hit + 1 WHERE id = $1",
        &[&id],
    )
    .await?;
