...
use crate::{
    db::{category, topic}, error::AppError, handler::{get_client, log_error, render, HtmlView}, view::frontend::topic::{ArchiveList, Detail, List}, AppState, Result
};
...
pub async fn detail(
    ...
    let item = topic::detail(&client, id)
        .await
        .map_err(log_error(handler_name))?;
    let tmpl = Detail {
        cats,
...
pub async fn archive(
    ...
    let dt = NaiveDateTime::parse_from_str(&dt, "%Y年%m月%d %H:%M:%S").map_err(AppError::from).map_err(log_error(handler_name))?;
